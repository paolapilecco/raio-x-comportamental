  ...
  return (
    <div className="bg-card rounded-2xl border border-border/30 shadow-sm overflow-hidden">
      <button
        ...
        className="w-full px-5 py-4 flex items-center gap-3 hover:bg-secondary/20 transition-colors"
      >
...
/* ─── Progress Reinforcement Messages ──────────────── */
function getProgressMessage(completedCount: number, totalCount: number): string | null {
  if (completedCount === 0) return null;
  if (completedCount === 1 && totalCount > 1) return 'Você já está diferente do seu padrão inicial. Isso já quebra parte do ciclo.';
  if (completedCount === 2 && totalCount === 3) return 'Duas fases concluídas. Seu cérebro já tem um caminho diferente. Falta consolidar.';
  if (completedCount === totalCount) return 'Você executou o que normalmente não executa. O padrão está mais fraco do que antes.';
  return null;
}
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const planStalled = actionPlan.days.length > 0 &&
      actionPlan.days[0]?.status === 'completed' &&
    ...
    const hasIncomplete = actionPlan.days.some(d => d.status === 'in_progress');
    if (hasIncomplete) {
      trackEvent({ userId: user.id, event: 'retest_alert_viewed', metadata: { type: 'task_incomplete', trigger: 'dashboard_load' } });
    }
  ...
  return (
    <AppLayout>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-10 space-y-5 sm:space-y-6">
          ...
          {focusMode && (
            ...
            >
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-card border border-border/40 shadow-lg text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-all active:scale-[0.97]"
              >
        ...
        {!focusMode && progressMessage && actionPlan.stats.completed_days > 0 && (
          <motion.div
            initial={{ opacity: 0, scale: 0.96 }}
            animate={{ opacity: 1, scale: 1 }}
            className="rounded-2xl bg-green-500/[0.04] border border-green-500/15 px-5 py-3.5 flex items-start gap-3"
          >
        ...
        {/* ═══════════ 4) NEXT STEP — DOMINANT CTA (always visible) ═══════════ */}
        {!loading && (
          <motion.section
            {...fadeIn}
            transition={{ ...fadeIn.transition, delay: 0.02 }}
          >
            <JourneyNextStep
        ...
        {/* ═══════════ 5 & 6) TASKS — Action Plan (always visible, supports focus) ═══════════ */}
        {!actionPlan.loading && actionPlan.days.length > 0 && (
          <motion.section {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.04 }}>
            <ActionPlanCard
              plan={actionPlan}
              behavioralMemory={centralProfile?.behavioral_memory}
        ...
        {/* ═══════════ SECONDARY SECTIONS — hidden in focus mode ═══════════ */}
        <AnimatePresence>
          {!focusMode && (
            <motion.div
              initial={{ opacity: 1 }}
              ...
              className="space-y-5"
            >
              {/* ═══════════ 2) PROFILE SUMMARY — Collapsible ═══════════ */}
              {hasData && latestResult && (
                <motion.section {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.06 }}>
                  <CollapsibleSection
                    title="Seu padrão atual"
                    icon={Brain}
                    defaultOpen={false}
                    badge={
                      <span className="text-[0.6rem] font-bold px-2 py-0.5 rounded-md mr-2"
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      {topTendency && (
                        <div className="border-t border-border/10 pt-3">
                          <p className="text-[0.55rem] uppercase tracking-wider text-muted-foreground/60 font-semibold mb-1.5">Principal tendência</p>
                      ...
                      {centralProfile?.core_pain && (
                        <div className="rounded-xl bg-destructive/[0.04] border border-destructive/10 px-4 py-2.5">
              ...
              {/* ═══════════ 3) JOURNEY PROGRESS — Compact ═══════════ */}
              {hasData && actionPlan.days.length > 0 && (
                <motion.section {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.08 }}>
                  <div className="bg-card rounded-2xl border border-border/30 shadow-sm p-4 sm:p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {actionPlan.stats.all_completed && (
                        <div className="px-2.5 py-0.5 rounded-full bg-green-500/10 text-green-600 text-[0.6rem] font-bold">✓ Completo</div>
                      )}
                    </div>
              ...
              {hasData && (
                <motion.section {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.16 }}>
                  <CollapsibleSection
                    ...
                    icon={History}
                    badge={
                      <span className="text-[0.6rem] text-muted-foreground/50 font-medium mr-2">
                        {sessionCount} leitura{sessionCount !== 1 ? 's' : ''}
                      </span>
                    }
                  >
                    <div className="space-y-3">
                      ...
                      {!showFull && sessionCount > 1 && (
                        <div className="rounded-xl bg-secondary/20 border border-border/10 px-4 py-3 text-center">
                          ...
                          <p className="text-[0.6rem] text-muted-foreground/60 font-medium">
                            Comparação entre testes disponível no processo completo.
                          </p>
                        </div>
                      )}
                      ...
                      {showFull && sessionCount >= 2 && (
                        <button onClick={() => navigate('/history')}
                          className="w-full py-2.5 rounded-xl text-xs font-semibold bg-primary/10 text-primary hover:bg-primary/15 transition-all active:scale-[0.97] flex items-center justify-center gap-2">
              ...
              {/* ═══════════ TESTS CATALOG — Collapsible ═══════════ */}
              {modules.length > 0 && (
                <motion.section {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.18 }}>
                  <CollapsibleSection title="Módulos de análise" icon={Brain}>
                    <div className="space-y-3">
                        ...
                        {modules.slice(0, 6).map((mod) => {
                          ...
                          return (
                            <button
                              ...
                              className="bg-secondary/20 rounded-xl p-3 border border-border/15 hover:border-border/30 hover:shadow-sm transition-all duration-200 active:scale-[0.97] text-left"
                            >
                              <div className="flex items-center gap-2 mb-1.5">
                                <div className="w-6 h-6 rounded-lg bg-secondary/60 flex items-center justify-center">
                                  {canAccess ? <Icon className="w-3 h-3 text-primary" /> : <Lock className="w-3 h-3 text-muted-foreground/30" />}
                                </div>
                              ...
                              {!canAccess && (
                                <span className="text-[0.5rem] text-muted-foreground/40 font-medium mt-0.5 block">Premium</span>
                              )}
                            </button>
                          );
                        })}
              ...
              {role === 'super_admin' && (
                <div className="flex items-center gap-3 flex-wrap pt-4 border-t border-border/10">
                  ...
                  {!previewMode && (
                    <button
                      onClick={async () => {
                        if (!user || !window.confirm('Tem certeza? Isso apagará TODOS os seus dados de teste.')) return;
                        setResetting(true);
                        try {
                          const uid = user.id;
