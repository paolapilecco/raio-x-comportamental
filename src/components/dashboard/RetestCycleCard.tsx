...
import { CalendarClock, ArrowDownRight, ArrowUpRight, Minus, RefreshCw, TrendingDown, TrendingUp } from 'lucide-react';
...
export function RetestCycleCard({ retest }: RetestCycleCardProps) {
    ...
    <motion.section {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.07 }}>
      <div className="bg-card rounded-2xl border border-border/30 shadow-[0_1px_3px_0_rgb(0_0_0/0.04)] p-6 sm:p-8">
        <div className="flex items-center gap-2.5 mb-5">
          ...
          <h3 className="text-base font-semibold text-foreground">Ciclo de Reteste</h3>
          <span className="text-[0.6rem] text-muted-foreground ml-auto">a cada 15 dias</span>
        </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {retest.improvementCount > 0 && (
                    <span className="text-green-500 font-medium">{retest.improvementCount} melhor{retest.improvementCount > 1 ? 'as' : 'a'}</span>
                  )}
                  {retest.improvementCount > 0 && retest.worsenedCount > 0 && ' · '}
                  {retest.worsenedCount > 0 && (
                    <span className="text-orange-500 font-medium">{retest.worsenedCount} pior{retest.worsenedCount > 1 ? 'es' : ''}</span>
                  )}
                  {retest.improvementCount === 0 && retest.worsenedCount === 0 && (
                    <span className="text-muted-foreground">Sem mudanças significativas</span>
                  )}
                </p>
              </div>
          ...
          {retest.scoreComparisons.length === 0 && (
            <div className="flex-1 flex items-center justify-center">
              <p className="text-xs text-muted-foreground text-center leading-relaxed">
                Faça seu segundo teste para ver a comparação de evolução entre as leituras.
              </p>
            </div>
          )}
        </div>
